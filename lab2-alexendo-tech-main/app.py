...
def generate_advice(student_id):
    ...
    try:
        ...
        app.logger.error(f"OpenAI call failed for student {student_id}: {e}")
        return jsonify({"error": "Upstream AI service failed"}), 502

    # 4. Save advice into the student dict
    student["advice"] = advice

    # 5. Return 200 with: {"id": ..., "major": "...", "advice": "..."}
    return jsonify({"id": student_id, "major": major, "advice": advice}), 200
...
def get_advice(student_id):
    ...
    # 3. Return 200 with: {"id": ..., "advice": "..."}
    return jsonify({"id": student_id, "advice": advice}), 200
